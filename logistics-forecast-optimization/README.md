# 2025年 MathorCup 数学建模竞赛D题：基于约束分段预测模型的货量运输优化调度
...
## 🧠 核心模型与解题思路
### 问题一：有约束的分段LSTM货量预测模型
针对物流货量的强周期性、时段性波动特征，创新性提出**带连续性约束的分段LSTM预测模型**：
1.  数据预处理：将数据按早班（21:00-次日6:00）、午班（11:00-14:00）做周期划分，再拆分高峰/低谷段；
...
## 📊 核心结果展示
### 1. 预测模型效果对比
![预测结果对比图](./04-图表结果/代码运行结果图/预测结果对比图.png)
*上图为原生LSTM、分段LSTM、有约束分段LSTM三种模型的预测结果与真实值对比，有约束分段模型对高峰段的预测精度提升最为显著。*

### 2. 可视化结果
![自有与外部车辆占比](./04-图表结果/代码运行结果图/自有与外部车辆占比.png)
*优化后自有车辆使用率达63.7%，低货量线路通过串点合并实现装载率提升至90%以上。*
